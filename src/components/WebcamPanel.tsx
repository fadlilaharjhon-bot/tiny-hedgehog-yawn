...
import { Info, Video, VideoOff, Loader2, Timer, Lamp, Hand } from "lucide-react";
...
const WebcamPanel = ({ onSetDelayTimer }: WebcamPanelProps) => {
  ...
  const lastGestureValue = useRef<number | null>(null);
    ...
    const createHandLandmarker = async () => {
      try {
        ...
        const landmarker = await HandLandmarker.createFromOptions(vision, {
          ...
          runningMode: "VIDEO",
          numHands: 2, // PENTING: Mengizinkan deteksi 2 tangan
        });
  ...
  const resetGestureState = () => {
    ...
    lastGestureValue.current = null;
    if (resetStageTimeout.current) clearTimeout(resetStageTimeout.current);
  };

  const processGesture = (value: number) => {
    ...
    if (gestureStage === "selecting_lamp") {
      ...
      let delayMinutes = 0;
      if (value === 0) delayMinutes = 5;
      else if (value === 1) delayMinutes = 10;
      else if (value === 2) delayMinutes = 20;
      else if (value === 5) delayMinutes = 60;
  ...
  const predictWebcam = () => {
    ...
    if (canvasCtx && video.currentTime !== lastVideoTime.current) {
      ...
      let currentGestureValue = -1;
      if (gestureStage === 'selecting_lamp') {
        currentGestureValue = results.landmarks.length; // Hitung jumlah tangan
      } else {
        currentGestureValue = results.landmarks.length > 0 ? countFingers(results.landmarks[0]) : -1; // Hitung jari di tangan pertama
      }
      ...
      if (currentGestureValue !== lastGestureValue.current) {
        lastGestureValue.current = currentGestureValue;
        gestureStartTime.current = now;
      }

      const inCooldown = now - lastCommandTime.current < GESTURE_COOLDOWN_MS;
      if (inCooldown) {
        // Tampilkan pesan cooldown atau pesan sukses
      } else if (lastGestureValue.current !== -1 && lastGestureValue.current !== null) {
        const elapsedTime = now - gestureStartTime.current;
        if (elapsedTime >= VALIDATION_TIME_MS) {
          processGesture(lastGestureValue.current);
        } else {
          const progress = Math.min(Math.round((elapsedTime / VALIDATION_TIME_MS) * 100), 100);
          let actionText = "";
          if (gestureStage === 'selecting_lamp') {
            if (lastGestureValue.current === 1) actionText = "Pilih Kamar 1";
            else if (lastGestureValue.current === 2) actionText = "Pilih Kamar 2";
          } else {
            if (lastGestureValue.current === 0) actionText = "Set Timer 5m";
            else if (lastGestureValue.current === 1) actionText = "Set Timer 10m";
            else if (lastGestureValue.current === 2) actionText = "Set Timer 20m";
            else if (lastGestureValue.current === 5) actionText = "Set Timer 1j";
          }
      ...
      <CardContent className="space-y-4">
        <div className="relative aspect-video w-full bg-slate-900 rounded-md flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute top-2 left-2 bg-black/50 p-2 rounded-md flex items-center gap-2 text-base">
            {gestureStage === 'selecting_lamp' ? <Hand className="w-5 h-5 text-sky-400" /> : <Timer className="w-5 h-5 text-green-400" />}
        ...
        <Alert className="bg-slate-700/50 border-slate-600">
          ...
          <AlertDescription className="text-slate-300 space-y-1">
            ...
            <ul className="list-disc list-inside pl-2">
              <li><span className="font-mono">Angkat 1 Tangan:</span> Pilih Lampu Kamar 1</li>
              <li><span className="font-mono">Angkat 2 Tangan:</span> Pilih Lampu Kamar 2</li>
            </ul>
            <p className="font-bold mt-2">Tahap 2: Atur Timer (Gunakan Jari)</p>
            <ul className="list-disc list-inside pl-2">
