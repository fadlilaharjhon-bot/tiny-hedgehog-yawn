    ...
    <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700 text-white">
      ...
      <CardContent className="flex flex-col items-center justify-center p-6 gap-4">
        <svg viewBox="0 0 200 250" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-xs rounded-lg bg-slate-800 p-2">
          ...
          {/* Left Side: Carport */}
          <rect x="10" y="160" width="70" height="80" className={`transition-colors duration-500 ${isOn ? 'fill-slate-600' : 'fill-slate-700'}`} />
          <line x1="30" y1="160" x2="30" y2="240" className={`transition-colors duration-500 ${isOn ? 'stroke-slate-500' : 'stroke-slate-600'}`} strokeWidth="1" />
          <line x1="50" y1="160" x2="50" y2="240" className={`transition-colors duration-500 ${isOn ? 'stroke-slate-500' : 'stroke-slate-600'}`} strokeWidth="1" />
          <line x1="70" y1="160" x2="70" y2="240" className={`transition-colors duration-500 ${isOn ? 'stroke-slate-500' : 'stroke-slate-600'}`} strokeWidth="1" />

          {/* Right Side: Garden */}
          <rect x="140" y="160" width="55" height="80" className={`transition-colors duration-500 ${isOn ? 'fill-green-900/50' : 'fill-slate-700'}`} />
          <circle cx="155" cy="180" r="5" className={`transition-colors duration-500 ${isOn ? 'fill-green-700' : 'fill-slate-600'}`} />
          <circle cx="175" cy="190" r="6" className={`transition-colors duration-500 ${isOn ? 'fill-green-700' : 'fill-slate-600'}`} />
          <circle cx="160" cy="210" r="4" className={`transition-colors duration-500 ${isOn ? 'fill-green-700' : 'fill-slate-600'}`} />
          <circle cx="180" cy="225" r="5" className={`transition-colors duration-500 ${isOn ? 'fill-green-700' : 'fill-slate-600'}`} />
